...
export default function EnergyPage() {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div>
        <h1 className="text-3xl font-bold text-white">에너지 분석</h1>
        <p className="text-slate-400 mt-2">전력 소비 비교 및 투자 회수 기간 산출</p>
      </div>
      ...
      <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            <span className="text-3xl font-bold text-cyan-400">{targetFlow}</span>
      ...
      <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center">
            ...
            <InfoTooltip title="상사법칙 (Affinity Laws)">
              <p><strong>상사법칙이란?</strong></p>
              <p>펌프의 회전수(속도)와 유량, 양정, 전력 사이의 관계를 설명하는 법칙입니다.</p>
              <div className="mt-3 space-y-2">
                <div className="flex items-center gap-2">
                  <span className="text-cyan-400 font-mono text-sm">Q₂/Q₁ = n₂/n₁</span>
                  <span className="text-slate-400 text-xs">유량은 회전수에 비례</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-amber-400 font-mono text-sm">H₂/H₁ = (n₂/n₁)²</span>
                  <span className="text-slate-400 text-xs">양정은 회전수의 제곱</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-green-400 font-mono text-sm">P₂/P₁ = (n₂/n₁)³</span>
        ...
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={chartData} layout="vertical" margin={{ top: 10, right: 60, left: 80, bottom: 10 }}>
            ...
            <XAxis
              ...
              label={{ value: '전력 (kW)', position: 'insideBottom', offset: -5, fill: '#94a3b8', fontSize: 12 }}
            />
            <YAxis
              type="category"
              dataKey="name"
          ...
          {chartData.map((item, index) => (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                {item.saving > 0 && (
                  <span className="text-green-400 text-sm font-semibold">-{item.saving.toFixed(1)}%</span>
                )}
              </div>
              <p className="text-2xl font-bold text-white mb-2">
                {item.power.toFixed(1)} <span className="text-sm text-slate-400">kW</span>
              </p>
      ...
      <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700">
        <div className="flex items-center mb-6">
          <h3 className="text-lg font-semibold text-white">ROI 계산 / 절감 효과</h3>
          <InfoTooltip title="ROI (Return on Investment)">
            <p><strong>투자 회수 기간이란?</strong></p>
            <p>인버터 도입 비용을 에너지 절감으로 회수하는 데 걸리는 시간입니다.</p>
            <div className="mt-3">
              <p className="text-cyan-400 font-mono text-xs">ROI = 인버터 비용 / 연간 절감 비용</p>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div>
            <label className="block text-xs font-medium text-slate-500 mb-1">인버터 비용</label>
            <div className="relative">
              <input
                type="number"
                value={roiInputs.inverterCost / 10000}
                onChange={(e) => setRoiInputs({ ...roiInputs, inverterCost: Number(e.target.value) * 10000 })}
                className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 pr-12 text-white text-sm focus:outline-none focus:border-cyan-500"
              />
          ...
          <div>
            <label className="block text-xs font-medium text-slate-500 mb-1">전기요금</label>
            <div className="relative">
              <input
                type="number"
                value={roiInputs.electricityRate}
                onChange={(e) => setRoiInputs({ ...roiInputs, electricityRate: Number(e.target.value) })}
                className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 pr-16 text-white text-sm focus:outline-none focus:border-cyan-500"
              />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-slate-900/50 rounded-lg p-4 border border-slate-700/50">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-semibold text-slate-300">절감량</span>
            </div>
          ...
          <div className="bg-slate-900/50 rounded-lg p-4 border border-slate-700/50">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-semibold text-slate-300">절감 비용</span>
            </div>
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-xs text-slate-500">연간</span>
              ...
              <div className="flex justify-between items-center">
                ...
                <span className={`text-sm font-semibold ${roiResult && roiResult.fiveYearSaving > 0 ? 'text-green-400' : 'text-red-400'}`}>
              ...
              <div className="flex justify-between items-center">
                ...
                <span className={`text-sm font-semibold ${roiResult && roiResult.tenYearSaving > 0 ? 'text-green-400' : 'text-red-400'}`}>
          ...
          <div className={`rounded-lg p-4 border ${
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-semibold text-slate-300">투자 회수</span>
            </div>
            {roiResult?.roiYears != null ? (
              <div className="text-center py-2">
                <p className="text-3xl font-bold text-white">
                  {roiResult.roiYears.toFixed(1)}<span className="text-lg text-slate-400 ml-1">년</span>
                </p>
              ...
              <div className="text-center py-4">
                <p className="text-lg font-semibold text-slate-500">절감량 없음</p>
              </div>
            )}
          </div>
        </div>
